...
import { BriefcaseIcon, MapPinIcon, MoreHorizontalIcon, PencilIcon, UserIcon } from "lucide-react";
...
export default function Profile() {
  ...
  // Extract from profileMetadata without mock defaults to keep profile 100% authentic
  const bio = user.profile.profileMetadata?.bio;

  // Conditionally render Location details if present in profileMetadata
  const country = user.profile.profileMetadata?.country;
  const state = user.profile.profileMetadata?.state;
  const city = user.profile.profileMetadata?.city;
  const postalCode = user.profile.profileMetadata?.postalCode;
  const hasLocation = !!(country || state || city || postalCode);

  // Conditionally render Professional details if present in profileMetadata
  const jobTitle = user.profile.profileMetadata?.jobTitle;
  const company = user.profile.profileMetadata?.company;
  const department = user.profile.profileMetadata?.department;
  const experience = user.profile.profileMetadata?.experience;
  const hasProfessional = !!(jobTitle || company || department || experience);

  // Format dates beautifully
  const memberSince = new Date(user.metadata.createdAt).toLocaleDateString(undefined, {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
  ...
  const lastLogin = user.metadata.lastLogin
    ? new Date(user.metadata.lastLogin).toLocaleString(undefined, {
        dateStyle: "medium",
        timeStyle: "short",
      })
    : "Never";
        ...
        <div className="mb-4 flex items-center gap-8 max-[24rem]:gap-4">
          <div className="flex items-center gap-4">
            <Avatar className="size-28 border max-[24rem]:size-20 sm:size-40">
              ...
              <AvatarFallback className="font-medium text-2xl">{initials}</AvatarFallback>
            </Avatar>
          </div>
          <div className="flex flex-col gap-0.5 sm:gap-1">
            <span className="font-medium text-lg tracking-tight sm:text-2xl">{displayName}</span>
            <span className="text-muted-foreground sm:text-lg">{email}</span>
          </div>
        </div>

        {bio && <p className="mt-0.5 line-clamp-4 text-muted-foreground">{bio}</p>}
        ...
        <div className="space-y-8 py-8">
          <div>
            ...
            <div className="grid grid-cols-1 *:-ms-px *:-mt-px *:border *:border-dashed *:px-4 *:py-3 md:grid-cols-2">
              <UserInfo label="Full Name" value={displayName} />
              <UserInfo label="Email" value={email} />
              <UserInfo label="Roles" value={user.identity.roles.map((r) => r.toUpperCase()).join(", ")} />
              <UserInfo label="Email Status" value={user.identity.isEmailVerified ? "Verified" : "Unverified"} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 *:-ms-px *:-mt-px *:border *:border-dashed *:px-4 *:py-3 md:grid-cols-2">
              <UserInfo label="Account Status" value={user.metadata.status.toUpperCase()} />
              <UserInfo label="Member Since" value={memberSince} />
              <UserInfo label="Last Login" value={lastLogin} />
              <UserInfo label="Default Workspace ID" value={user.metadata.defaultWorkspaceId || "None"} />
            </div>
          </div>
          ...
          {hasLocation && (
            <div>
              ...
              <div className="grid grid-cols-1 *:-ms-px *:-mt-px *:border *:border-dashed *:px-4 *:py-3 md:grid-cols-2">
                {country && <UserInfo label="Country" value={country} />}
                {state && <UserInfo label="State/Province" value={state} />}
                {city && <UserInfo label="City" value={city} />}
                {postalCode && <UserInfo label="Postal Code" value={postalCode} />}
              </div>
            </div>
          )}
          ...
          {hasProfessional && (
            <div>
              ...
              <div className="grid grid-cols-1 *:-ms-px *:-mt-px *:border *:border-dashed *:px-4 *:py-3 md:grid-cols-2">
                {jobTitle && <UserInfo label="Job Title" value={jobTitle} />}
                {company && <UserInfo label="Company" value={company} />}
                {department && <UserInfo label="Department" value={department} />}
                {experience && <UserInfo label="Years of Experience" value={experience} />}
              </div>
            </div>
          )}
        </div>
...
function UserInfo({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <h3 className="mb-1 font-medium text-muted-foreground text-sm">{label}</h3>
...
const Reddit = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 216 216" xmlnsXlink="http://www.w3.org/1999/xlink">
    <defs>
      <radialGradient
        cx="169.75"
        cy="92.19"
    ...
    <path
      ...
      fill="#ff4500"
    />
    <circle cx="169.22" cy="106.98" fill="url(#reddit__snoo-radial-gragient)" r="25.22" />
    <circle cx="46.78" cy="106.98" fill="url(#reddit__snoo-radial-gragient-2)" r="25.22" />
    <ellipse cx="108.06" cy="128.64" fill="url(#reddit__snoo-radial-gragient-3)" rx="72" ry="54" />
    <path
    ...
    />
    <circle cx="147.49" cy="49.43" fill="url(#reddit__snoo-radial-gragient-7)" r="17.87" />
    <path
